...
def MCengine_post_merger(N_mc,M1,M2,Z1,Z2,D_proj,prefix,C1=None,C2=None,del_mesh=100,TSM_mesh=200):
    '''
    ...
    while i < N_mc:
        ...
        # Draw a random angle between 0 and 20 degrees
        ALPHA = numpy.random.rand() * 45 * numpy.pi / 180  # Convert degrees to radians

        # Since not all alpha are equally likely.
        # alpha = (1 - numpy.cos(ALPHA)) * 45 * numpy.pi / 180  # Scale accordingly to the 20 degree range
        """corrected alpha for not 90 degree!"""
        max_angle_rad = numpy.radians(45)
        U = numpy.random.rand()
